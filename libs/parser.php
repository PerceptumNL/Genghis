    ...
    foreach ($arrays as $index => $piece){
        ...
        if (!is_array($piece)) {
            ...
            if ($piece[strlen($piece)-1] == '^'){
                ...
                $base = explode(',', $arrays[$index+1][0]);
                //print_r($base);
                $var = $base[0];
                $base = $base[1];
                //print_r($var);
                $final_string .= "(Math.log(". recursive_parsing(array($var)) . ")/Math.log(" . $base . "))";
                $power=1;
                //$final_string .=  "WE GOT LOG";
            } else {
                $final_string .= $piece;
            }
        } else {
...
function round_everything($string) {
    /*$string = 'The quick brown fox jumped over the lazy dog.';
    $patterns = array();
    $patterns[0] = '/<var>/';
    $patterns[1] = '/(.*?)/';
    $patterns[2] = '/<\/PRODUCT_LIST>/s';
    $replacements = array();
    $replacements[2] = 'bear';
...
function process_and_parse(){
    ...
    foreach ($allVariables as $id => $properties){
        ...
        } elseif ($properties['type'] == 'float'){
            $allVariables[$id]['min'] = $result['float_min'];
            $allVariables[$id]['max'] = $result['float_max'];
    ...
    $super_String = $super_String . $problem_code.round_everything(find_and_parse_latex($statement));
    //-------------------------------SOLUTION_CHECKER--------------------
    if ($check) {
        if ($round) {
    ...
    $fileLocation = EXERCISES_PATH."/exercises/".$course."_".$_SESSION['question_id'].".html";
